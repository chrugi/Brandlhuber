...
\desc{TIM gibt JAN grinsend den zweiten Becher Kaffee, den er versteckt gehalten
...
\dialog{Tim}{Du musst den Verkehrsparameter sehr schwer gewichten. Sonst hast du
  am Schluss eine Fussgängerzone.}

\dialog{Jan}{Das wäre doch auch nicht so schlecht. Aber es löst das Problem der
  öffentlichen Anbindung nicht.}

\subh{ALESSIA kommt dazu}

\dialog{ALlessia}{Guten Morgen ihr zwei! Naja mindestens dich, Jan, brauche ich
...
%
%\dialog{Gudzilla}{Ihre Forschung hat bis jetzt sehr wenige Früchte getragen. Ich
...
%
%\dialog{Brown}{Aber ich bin nun endlich unmittelbar vor dem grossen Durchbruch!
...
%
%\dialog{Gudzilla}{Ich habe mich doch bezüglich den zeitlichen Rahmenbedingungen
%  das letzte Mal klar und deutlich ausgedrückt, oder? Es tut mir leid, aber ich
%  kann hier keine Ausnahme machen. Ich kann nichts mehr für sie tun.}
%
...
\desc{Jan gibt Tim seinen Laptop. Sie probieren beide ein wenig unbeholfen herum
...
\exdialog{Alessia}{So. Nach einem Neustart müsste es funktionieren.}{überzeugt}

\desc{Jan startet den Computer neu und gibt sein Passwort ein.}

\exdialog{Alessia}{Ehrlich? Password.1234?}{augenrollend}

\desc{Jan erwidert verlegen Alessias Blick. Er startet das Programm, welches
  sofort wieder abstürzt.}

\exdialog{Alessia}{Hmm... Das verstehe ich jetzt nicht. Vielleicht gehst du doch
  lieber zum Helpdesk.}{verlegen}


% ############################################################################ %
\scene{int}{Helpdesk - ETH}{Nachmittag}{DR. STANISLAV BROWN ist schlecht
...
\dialog{Jan}{Hallo. Ich habe ein Problem mit `Dreamfetcher'. Das Programm stürzt
  immer ab, wenn ich versuche die Parametergewichtung zu ändern.}

\exdialog{Brown}{Hast du versucht den Computer neu zu starten?}{sichtlich
  genervt}
...
\dialog{Brown}{Hm. Ich fürchte, ich kann da nichts machen. Es scheint
  tatsächlich ein grundlegenderes Problem zu sein. Ich kann es mir höchstens im
  Detail ansehen und sehen, was ich machen kann. Aber ich fürchte, du musst
  deinen Computer bis heute Abend hier lassen.}

\dialog{Jan}{Aber das geht nicht. Ich habe morgen eine Kritik und muss noch viel
  arbeiten.}
...
\desc{Jan scheint entäuscht, dass es keine einfache Lösung für sein Problem zu
...
\dialog{Jan}{Also gut. Wenn sonst nichts geht, müssen wir es eben so versuchen.}
...
\exdialog{Brown}{Hallo Jan.\\ Ich habe deinen Computer nun fertig. Du kannst ihn
  nun abholen.}{über das Telefon}
...
\dialog{Brown}{Mit der alten Installation war nichts mehr anzufangen. Ich habe
  dir deshalb die neueste Version installiert. Mit dieser hast du nun auch nicht
  mehr das Problem mit den Lizenzen.}

\exdialog{Jan}{Vielen Dank! Ich wollte schon aufgeben.\\Aber unter Umständen
  muss ich das nun auch. Bis morgen bleibt nicht mehr viel Zeit, und es ist noch
  eine Menge zu tun.}{Glücklich, aber trotzdem mit \\einem Anflug von
  Resignation}
...
\desc{Jan nimmt die Maus in die Hand und versucht in der neuen Oberfläche einige
...
\dialog{Mira}{Die Sprachsteuerung ist nun aktiviert.\\Wo soll Ihr neues Projekt
  liegen?}
...
\dialog{Tim}{Hat es also noch geklappt mit deinem Computer gestern? Brändi wird
  es sicherlich verstehen, dass du unter diesen Umständen den Entwurf nicht
  vollenden konntest.}
...
\desc{Als Jan seine Ausdrucke zusammen und zugeschnitten hat, schaut er auf die
  Uhr.}

\dialog{Jan}{Wenn wir vor der Kritik noch eine Zigarette rauchen wollen, müssen
  wir JETZT gehen.}
...
\exdialog{Jan}{Nun, ich will nicht allzu sehr ins Detail meiner Parameter
  ...
  integration vom Neuen zu erreichen. Sonst wäre meine Idee von Grund auf nicht
...
\dialog{Jan}{Mensch bin ich froh, dass es vorüber ist. Aber es ist ja zum glück
  recht gut gelaufen.}

\dialog{Tim}{Ja Jan. Wie hast du das geschafft? Ich meine, als ich mich gestern
  mit dir zusammengesetzt habe, war dein Projekt noch nirgends.. Und heute?
  Heute überflügelst du mich.}
...
\dialog{Tim}{Na etwa das übliche. Es gab Bier, die Leute waren ausgelassen und
  um zwei sind die letzten nach hause gegangen, wie ich gehört habe. Wir sind um
...
% ############################################################################ %
\scene{int}{Browns Labor}{Abend}{Man sieht eine dunkle Silhouette vor seinem
